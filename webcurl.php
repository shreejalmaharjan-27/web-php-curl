...
{   
    ...
    if (preg_match("/^[.A-Za-z0-9-]+$/", $domain)) {
        $nth = "";
        if(isset($_POST['curl1']) && array_key_exists($_POST['curl1'], $actions) ? $array[$nth] : '') {
         if(isset($_POST['curl2']) && array_key_exists($_POST['curl2'], $actions) ? $array[$nth] : '') { 
         if(isset($_POST['curl3']) && array_key_exists($_POST['curl3'], $actions) ? $array[$nth] : '') {            
                //connect ipv4/v6
